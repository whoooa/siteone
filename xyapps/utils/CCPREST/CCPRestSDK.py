...
class REST:
    ...
    def log(self, url, body, data):
        add_log.info("SMS SENT: %s\n********************************\n",
                     json.dumps({"url": url, "body": body, "data": data}))
    ...
    def sendTemplateSMS(self, to, datas, tempId):
        ...
        sig = md5(signature).hexdigest().upper()
        # 拼接URL
        url = "https://%s:%s/%s/Accounts/%s/SMS/TemplateSMS?sig=%s" % (self.ServerIP, self.ServerPort,
                                                                       self.SoftVersion, self.AccountSid, sig)
        # 生成auth
        src = self.AccountSid + ":" + self.Batch
        auth = base64.encodestring(src).strip()
        req = urllib.request.Request(url)
