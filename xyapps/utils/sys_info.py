...
def sys_information():
    try:
        ...
        # nginx状态
        # the_url = 'http://' + ALLOWED_HOSTS[0] + '/nginx-status-wl'
        # nginx_req = urllib2.Request(the_url)
        # nginx_res = urllib2.urlopen(nginx_req)
        # nginx_res_data = nginx_res.read().split("\n")
        # nginx_res.close()
        # nginx = "<br>".join([nginx_res_data[0], nginx_res_data[1] + nginx_res_data[2], nginx_res_data[3]])
        nginx = "-"
